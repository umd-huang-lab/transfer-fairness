#Source: 'https://github.com/VectorInstitute/laftr'
import numpy as np
import torch
...
#TODO: change back numpy or keep in tensor form?
def TP(Y, Ypred):
    #return sum(np.multiply(Y, np.round(Ypred)))#.astype(np.float32)
    return sum(torch.multiply(Y, Ypred.long()))

def FP(Y, Ypred):
    #return np.sum(np.multiply(np.logical_not(Y), np.round(Ypred)))#.astype(np.float32)
    return sum(torch.multiply(torch.logical_not(Y), Ypred.long()))

def TN(Y, Ypred):
    #return np.sum(np.multiply(np.logical_not(Y), np.logical_not(np.round(Ypred))))#.astype(np.float32)
    return sum(torch.multiply(torch.logical_not(Ypred.long()), torch.logical_not(Y)))

def FN(Y, Ypred):
    #return np.sum(np.multiply(Y, np.logical_not(np.round(Ypred))))#.astype(np.float32)
    return sum(torch.multiply(Y, torch.logical_not(Ypred.long())))
#TODO: change back numpy or keep in tensor form?

def FP_soft(Y, Ypred):
    return np.sum(torch.multiply(np.logical_not(Y), Ypred))#.astype(np.float32)
...
def errRate(Y, Ypred):
    return (FP(Y, Ypred) + FN(Y, Ypred)) / Y.shape[0]
